...
m4work <- read_csv("fits/lhs_all/sc4/worked.csv")[,-1]
max(m4work$ll)
ma <- m4work %>% filter(ll > -630, rel_fit < 1) # hard to choose
m <- as.numeric(ma[1,])

Initial.Values = c(mu2 = m[2], mu5 = m[3], mu6 = m[4],
                   mu7 = m[5], mu8 = m[6], mu10 = m[7],
                   gamma2 = m[8], gamma5 = m[9],
                   gamma6 = m[10], gamma7 = m[11],
                   gamma8 = m[12], gamma10 = m[13],
                   f2 = m[14], f5 = m[15], f6 = m[16],
                   f7 = m[17], f8 = m[18], f10 = m[19],
                   grow = m[20], rel_fit = m[21])
# check not -Inf
# run_sim_logPosterior(Initial.Values) # -624
