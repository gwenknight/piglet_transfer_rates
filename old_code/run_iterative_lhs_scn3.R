...
m3work <- read_csv("fits/lhs_all/sc3/worked.csv")[,-1]
max(m3work$ll)
ma <- m3work %>% filter(ll > -600, rel_fit < 1)
m <- as.numeric(ma[1,]) # hard to choose between

Initial.Values = c(mu = m[2],
                     gamma = m[3],
                     f2 = m[4], f5 = m[5], f6 = m[6], 
                     f7 = m[7], f8 = m[8], f10 = m[9], 
                     grow = m[10], 
                     rel_fit = m[11])
# check not -Inf
# run_sim_logPosterior(Initial.Values) # -634
